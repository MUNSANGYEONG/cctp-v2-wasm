#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# skip-go IBC Callbacks 버전 배포:
#   1) ibc_adapter (ibc_callbacks) 코드 store → instantiate (임시 entry_point 주소)
#   2) entry_point 코드 store → instantiate (adapter 주소 참조)
#   3) adapter migrate → 실제 entry_point 주소로 갱신
#
# 사용법: scripts/deploy_skip_callbacks.sh
...
# wasm 경로
CALLBACKS_ADAPTER_WASM="${CALLBACKS_ADAPTER_WASM:-${ROOT_DIR}/artifacts/skip_go_ibc_adapter_ibc_callbacks.wasm}"
ENTRY_POINT_WASM="${ENTRY_POINT_WASM:-${ROOT_DIR}/artifacts/skip_go_entry_point.wasm}"

# deployments/<chain>_skip_callbacks.json 에 별도 기록
RECORD_FILE="${DEPLOY_DIR}/${CHAIN_ID}_skip_callbacks.json"
mkdir -p "${DEPLOY_DIR}"
if [ ! -f "${RECORD_FILE}" ]; then
  jq -n --arg cid "${CHAIN_ID}" --arg bin "${BINARY}" '{
    chain_id: $cid, binary: $bin, variant: "ibc_callbacks",
...
# ── 1) ibc_callbacks adapter 코드 store ─────────────────────────────────────
log "[1/5] ibc_callbacks adapter 코드 store"
store_code "${CALLBACKS_ADAPTER_WASM}"
...
# ── 2) ibc_callbacks adapter instantiate (임시 주소 사용) ───────────────────
# entry_point 주소를 미리 알 수 없으므로 배포자 주소를 임시로 사용.
# 이후 migrate 로 실제 entry_point 주소로 교체한다.
log "[2/5] ibc_callbacks adapter instantiate (임시 entry_point=${SENDER_KEY_ADDR})"
...
ADAPTER_INST_JSON="$(broadcast tx wasm instantiate "${ADAPTER_CODE_ID}" "${ADAPTER_INIT_MSG}" \
  --label "skip-go-ibc-adapter-ibc-callbacks" --admin "${ADMIN_ADDR}")"
...
EP_INST_JSON="$(broadcast tx wasm instantiate "${EP_CODE_ID}" "${EP_INIT_MSG}" \
  --label "skip-go-entry-point-ibc-callbacks" --admin "${ADMIN_ADDR}")"
...
echo ""
echo "=== skip-go ibc_callbacks 배포 완료 ==="
